...
@pytest.mark.anyio
async def test_get_stoch_decisions(sample_dataframe, sample_stoch_dataframe, dbsession: AsyncSession):
    calculator = StochCalculator()
    period = 'ALL'
    ...
    with (
        patch('backend.utils.moex.moex_reader.MoexReader.get_company_history') as mock_method,
        patch('backend.utils.stoch.stoch_calculator.StochCalculator._generate_stoch_df') as mock_stoch
    ):
        ...
        with report_time("Generate decisions"):
            decisions = await calculator.get_companies_stoch_decisions(companies, period)
