...
<script>
import { getCategoriesSimple, postData, putData, deleteData, endpoints } from '@/api/invmos-back'
export default {
  name: 'CatalogSimple',
  props: {
    type: {
  ...
  methods: {
    fetchList() {
      getCategoriesSimple(this.type).then(data => {
        this.list = data
      })
    },
    handleAdd() {
    ...
    addItem() {
      postData(endpoints.CATEGORIES_SIMPLE, this.temp, false, { type: this.type })
        .then(() => {
          this.fetchList()
          this.dialog = false
        })
    ...
    editItem() {
      putData(endpoints.CATEGORIES_SIMPLE, this.temp, false)
        .then(() => {
          this.fetchList()
          this.dialog = false
        })
    ...
    handleDelete(id) {
      ...
      deleteData(endpoints.CATEGORIES_SIMPLE, { id: id })
        .then(() => {
          this.active = []
          this.selected = []
          this.fetchList()
